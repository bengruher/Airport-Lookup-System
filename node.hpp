...
namespace kdtree {
    ...
    public:
         ...
         *  @return Initialized node instance.
         */
		 bool myFLat(airports a, airports b){
			 return a.lat < b.lat;
		 }
        ...
        node(T *points, int size, int depth = 0) {
            ...
            if (is_even) {
                std::sort(*points, points + size, myFLat);
            } else {
                std::sort(*points, points + size, [](T const& a, T const& b) { return a.lon < b.lon; });
            }
        ...
        inline double distance(node<T> *node) {
          double lat1 = this->point.lat;
          double lat2 = node->point.lat;
          double lon1 = this->point.lon;
          double lon2 = node->point.lon;
            double theta, dist;
            theta = lat1 - lon1;
        ...
        node<T> * nearest(node<T> *query, const int depth = 0) {
            ...
            bool is_even = !(depth & 1);
            double dx = query->point.lat - this->point.lat;
            double dy = query->point.lon - this->point.lon;
            double distance = is_even ? std::sqrt(dx * dx) : std::sqrt(dy * dy);
            
            // Find the nearest node
            bool left = is_even ? (this->point.lat > query->point.lat) : (this->point.lon > query->point.lon);
            node<T> *leaf = NULL;
